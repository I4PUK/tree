...
/// Добавление детей в дерево
func readChildDir(path string, relativeDir string, parent *FileProps) {
	...
	for i, file := range files {
		...
		if file.IsDir() {
			...
			readChildDir(filepath.Join(path, parent.Children[i].Name), parent.Children[i].RelativePath, &parent.Children[i])
		} else {
...
/// Первая итерация программы
func readTopDir(path string) {
	/// Дерево папок
	result := FileProps{
		Name:         path,
		RelativePath: path + "/",
	...
	readChildDir(path, result.RelativePath, &result)
